...
iterateContributionTypes.mockResolvedValue({
  contributionsByRepository,
});
...
describe('iterateContributionPeriods', () => {
  ...
  it("Should call iterateContributionTypes once when there's only 1 contributing period", async () => {
    ...
    expect(iterateContributionTypes).toHaveBeenCalledTimes(
      memberWith1Period.periods.length,
    );
  });
  ...
  it('Should call iterateContributionTypes for each user contributing period', async () => {
    ...
    expect(iterateContributionTypes).toHaveBeenCalledTimes(
      memberWith2Periods.periods.length,
    );
    expect(iterateContributionTypes).toHaveBeenNthCalledWith(1, {
      ...baseArgs,
      period: memberWith2Periods.periods[0],
    });
    expect(iterateContributionTypes).toHaveBeenNthCalledWith(2, {
      ...baseArgs,
      period: memberWith2Periods.periods[1],
    });
  });
});
