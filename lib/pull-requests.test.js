...
contributions.getContributionStats.mockResolvedValue({
...
describe('getData', () => {
  ...
  it('Should call getContributionStats with only pull request as contribution type', async () => {
    ...
    expect(contributions.getContributionStats).toHaveBeenCalledWith({
      ...args,
  ...
  it('Should return pullRequestsByRepo, totalContributions and members (to be consumed by normalise func) ', () => {
    expect(pullRequests.contributionsByRepo.length).toBe(
      contributionsByRepository.length,
    );
...
describe('normalise', () => {
  ...
  it('Should return contributors ordered by number of repo contributions', () => {
    const contributionsCount =
      contributionsByRepository[1].contributions.contributors;

    [contributor1, contributor2] = repo2.contributors;
    expect(contributionsCount[contributor1]).toBeGreaterThan(
      contributionsCount[contributor2],
    );
  });
  ...
  it('Should return a list of the topics with contributions and associated repos', () => {
    ...
    const repoTopicsCount = flatten(
      contributionsByRepository.map((contribution) => {
        return get(contribution, 'repository.repositoryTopics.nodes').map(
          ({ topic: { name } }) => name,
        );
      }),
    ).reduce((acc, topic) => {
...
describe('summariseContributions', () => {
  it('Should summarise contributions considering all repositories', () => {
    const { repos, repoCount, contributionsCount } = summariseContributions(
      normalisedData,
    );
