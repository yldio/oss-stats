const { map, size, flatMap, uniq, keyBy, orderBy } = require('lodash');
...
async function getData({ org, token, members }) {
  ...
  const pullRequestsByRepo = pullRequests.contributionsByRepository.map(
  ...
  return {
    contributionsByRepo: pullRequestsByRepo,
    totalContributions: pullRequests.totalContributions,
    members,
  };
}
  ...
  normalised.members = keyBy(
    members.map((m) => {
      return {
        ...
        repositoriesContributedTo: pullRequestsByAuthor[m.login]
          ? orderBy(
